...
//  Copyright (c) 2002 Quentin D. Carnicelli. All rights reserved.
//
//  Feedback welcome at qdc@eckysoft.com
...
- (OSStatus)handleHotKeyEvent: (EventRef)inEvent
{
	...
	while( (hotKey = [enumerator nextObject]) != nil )
	{
		if( hotKey->mId == hotKeyID.id ) {
			switch( GetEventKind( inEvent ) )
			{
				case kEventHotKeyPressed:
					[self _hotKeyDown: hotKey]; break;
					
				case kEventHotKeyReleased:
					[self _hotKeyUp: hotKey]; break;
...
- (BOOL)_registerHotKeyIfNeeded: (_HotKeyData*)hk
{
		...
		[combo isValid] == YES )
	{
		...
		hk->mId = _nextHotKeyId;
		
		keyID.signature = kHotKeyCenterSignature;
		keyID.id = _nextHotKeyId;
		short modifiers = [combo modifiers];
		if (modifiers == 0)
		{
			modifiers = btnState; // HACK - allows function keys without modifiers to be registered
		}
		err = RegisterEventHotKey( [combo keyCode], modifiers, 
