...
UInt32 _strtoul(char *str, int size)
{
    UInt32 total = 0;
    int i;
    ...
    for (i = 0; i < size; i++)
    {
		UInt32 value = (unsigned char)str[i];
		UInt32 shift = ((size - 1 - i) * 8);
		UInt32 increment = value << shift;
		total += increment;
    }
    return total;
}
...
{
	float temp = (float)(((val.bytes[0] << 8) + val.bytes[1]) >> 2) / 64.0;
...
{
    printf("%u ", (unsigned int) _strtoul(val.bytes, val.dataSize));
}
...
kern_return_t SMCReadKey(UInt32Char_t key, SMCVal_t *val)
{
    ...
    inputStructure.key = _strtoul(key, 4);
    sprintf(val->key, key, nil);
...
UInt32 SMCReadIndexCount(void)
{
    ...
    SMCReadKey("#KEY", &val);
    return _strtoul(val.bytes, val.dataSize);
}
...
kern_return_t SMCPrintFans(void)
{
    ...
    totalFans = _strtoul(val.bytes, val.dataSize);
